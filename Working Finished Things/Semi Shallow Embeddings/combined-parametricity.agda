...
extractT : ∀{aΓ PΓ} → {Γ : Context aΓ PΓ}
  → {t : Term aΓ U₁} → {Pt : PTerm PΓ (PU₁ PΓ) t t}
  → Exp Γ U₁ (PU₁ PΓ) t Pt
  → Term aΓ U₁
extractT {_} {_} {_} {t} {Pt} e = t
extractPT : ∀{aΓ PΓ} → {Γ : Context aΓ PΓ}
  → {t : Term aΓ U₁} → {Pt : PTerm PΓ (PU₁ PΓ) t t}
  → Exp Γ U₁ (PU₁ PΓ) t Pt
  → PTerm PΓ (PU₁ PΓ) t t
extractPT {_} {_} {_} {t} {Pt} e = Pt

exT : Exp ∅ U₁ (PU₁ Pnil) _ _ -- (λ γ → (X : Set) → X → X) _
-- exT = EΠ₁ EU₀ (EΠ₁ {! cumu₀ (EVar same)  !} {!   !} )
exT = EΠ₁ EU₀ (EΠ₁ (cumu₀ (EVar same)) (cumu₀ (EVar (next same))))

exTerm : Exp ∅ (extractT exT) (extractPT exT) _ _
