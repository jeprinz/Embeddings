...
Implementation of "unquote-n" for STLC. unquote-n is a technique for normalization
which I invented, only to discover that it is almost the same as NbE. It is not
exactly the same, however. Rather than assuming that all function arguments
are applied, it uses a type called ArgsCount to keep track of how many arguments
there actually are. Therefore, it does NOT η-expand the way standared NbE does.
However, this is almost always a disadvantage. One other difference is the there
is no separate reification step, as the result type of evaluation (eval is like
unquote-n function) is already the final result.
...
Note: a possible alternate implementation would be to replace this with a type
which doesn't just count the number of arguments, but actually stores the
arguments.
...
mutual
  ...
  PUExp {A ⇒ B} (one {A} count) Γ
    = (GExp Γ A) → PUExp {B} count Γ
