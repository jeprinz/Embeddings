...
In this file, I make things simpler by only letting parametricity work on types
...
weakenTLots : ∀{Γ : Ctx} → Type nil → Type Γ
weakenTLots T γ = T tt
...
data tPCtx : (aΓ : Ctx) → PCtx aΓ aΓ → Set j where
  ∅ : tPCtx nil Pnil
  Econs : ∀{aΓ PΓ} → (ctx : tPCtx aΓ PΓ) → (T : Type aΓ)
    → (PT : PType PΓ T T)
    → tPCtx (cons aΓ T) (Pcons PΓ PT)

data tPVar : ∀{aΓ PΓ} → (Γ : tPCtx aΓ PΓ) → (T : Type aΓ)
  → (PT : PType PΓ T T) → (a : Term aΓ T) → (PTerm PΓ PT a a)
  → Set j where
  same : ∀{aΓ PΓ} → {Γ : tPCtx aΓ PΓ} → {T : Type aΓ} → {PT : PType PΓ T T}
    → tPVar (Econs Γ T PT) (weakenT T) (PweakenT PΓ PT PT) var (Pvar PΓ PT)
  next : ∀{aΓ PΓ} → {Γ : tPCtx aΓ PΓ} → {A T : Type aΓ}
    → {PA : PType PΓ A A} → {PT : PType PΓ T T}
    → {a : Term aΓ T} → {Pa : PTerm PΓ PT a a}
    → tPVar Γ T PT a Pa
    → tPVar (Econs Γ A PA) (weakenT T) (PweakenT PΓ PA PT)
        (weaken a) (Pweaken PΓ PA PT Pa)

data tCtx : (aΓ : Ctx) → Set j where
  ∅ : tCtx nil
  Econs : ∀{aΓ} → (ctx : tCtx aΓ) → (T : Type aΓ) → tCtx (cons aΓ T)

data tVar : ∀{aΓ} → (Γ : tCtx aΓ) → (T : Type aΓ)
  → (a : Term aΓ T) → Set j where
  same : ∀{aΓ} → {Γ : tCtx aΓ} → {T : Type aΓ}
    → tVar (Econs Γ T) (weakenT T) var
  next : ∀{aΓ} → {Γ : tCtx aΓ} → {A T : Type aΓ}
    → {a : Term aΓ T} → tVar Γ T a
    → tVar (Econs Γ A) (weakenT T) (weaken a)

data CtxModel : Set j where
  -- error : CtxModel
  termPterm : (Γ : Ctx) → (PΓ : PCtx Γ Γ) → tPCtx Γ PΓ → CtxModel
  term : (Γ : Ctx) → tCtx Γ → CtxModel
  parametricity : (Γ : Ctx) → tCtx Γ → CtxModel
  paraAtType : (Γ : Ctx) → tCtx Γ → CtxModel

data Model : CtxModel → Set j where
  -- error : Model error
  termPterm : ∀{Γ PΓ} → (T : Type Γ) → (t : Term Γ T)
    → (PT : PType PΓ T T) → (Pt : PTerm PΓ PT t t)
    → (EΓ : tPCtx Γ PΓ) → Model (termPterm Γ PΓ EΓ)
  term : ∀{Γ} → (T : Type Γ) → (t : Term Γ T)
    → (EΓ : tCtx Γ) → Model (term Γ EΓ)
  parametricity : ∀{Γ} → (EΓ : tCtx Γ) → Model (parametricity Γ EΓ)
  paraAtType : ∀{Γ} → (T : Type Γ) → (t : Term Γ T)
    → (EΓ : tCtx Γ) → Model (paraAtType Γ EΓ)

PTypeToType∅ : {T : Type nil}
  → PType Pnil T T → Type nil
PTypeToType∅ {T} PT = λ tt → (x y : T tt) → PT tt x y

data AppCases : (cm₁ cm₂ cm : CtxModel) → Model cm₁ → Model cm₂
  → Model cm → Set j where
  tP : ∀{Γ PΓ} → {EΓ : tPCtx Γ PΓ}
    → {A : Type Γ} → {B : Type (cons Γ A)}
    → {PA : PType PΓ A A} → {PB : PType (Pcons PΓ PA) B B}
    → {x : Term Γ (Π A B)} → {Px : PTerm PΓ (PΠ PΓ PA PB) x x}
    → {y : Term Γ A} → {Py : PTerm PΓ PA y y}
    → AppCases (termPterm Γ PΓ EΓ) (termPterm Γ PΓ EΓ) (termPterm Γ PΓ EΓ)
       (termPterm (Π A B) x (PΠ PΓ PA PB) Px EΓ) (termPterm A y PA Py EΓ)
       (termPterm (sub B y) (app x y) (Psub PΓ PA PB Py) (Papp PΓ PB Px Py) EΓ)
  t : ∀{Γ} → {EΓ : tCtx Γ}
    → {A : Type Γ} → {B : Type (cons Γ A)}
    → {x : Term Γ (Π A B)} → {y : Term Γ A}
    → AppCases (term Γ EΓ) (term Γ EΓ) (term Γ EΓ)
       (term (Π A B) x EΓ) (term A y EΓ) (term (sub B y) (app x y) EΓ)
  p : ∀{Γ} → {EΓ : tCtx Γ}
    → {T : Type nil} → {PT : PType Pnil T T}
    → {t : Term nil T} → {Pt : PTerm Pnil PT t t}
    → AppCases (parametricity Γ EΓ) (termPterm nil Pnil ∅) (paraAtType Γ EΓ)
        (parametricity EΓ) (termPterm T t PT Pt ∅) (paraAtType (weakenTLots (PTypeToType∅ PT)) {!   !} EΓ)
  -- p : ∀{Γ} → {EΓ : tCtx Γ}
  --   → {A : Type Γ} → {B : Type (cons Γ A)}
  --   → {x : Term Γ (Π A B)} → {y : Term Γ A}
  --   → AppCases (parametricity Γ EΓ) (parametricity Γ EΓ) (parametricity Γ EΓ)
  --      (parametricity (Π A B) x EΓ) (parametricity A y EΓ) (parametricity (sub B y) (app x y) EΓ)
  pt : ∀{Γ} → {EΓ : tCtx Γ}
    → {A : Type Γ} → {B : Type (cons Γ A)}
    → {x : Term Γ (Π A B)} → {y : Term Γ A}
    → AppCases (paraAtType Γ EΓ) (paraAtType Γ EΓ) (paraAtType Γ EΓ)
       (paraAtType (Π A B) x EΓ) (paraAtType A y EΓ) (paraAtType (sub B y) (app x y) EΓ)

data VarCases : (Γ : CtxModel) → (T : Model Γ)  → Set j → Set k where
  tP : ∀{Γ PΓ} → {EΓ : tPCtx Γ PΓ}
    → {T : Type Γ} → {PT : PType PΓ T T}
    → {t : Term Γ T} → {Pt : PTerm PΓ PT t t}
    → VarCases (termPterm Γ PΓ EΓ) (termPterm T t PT Pt EΓ) (tPVar {Γ} {PΓ} EΓ T PT t Pt)
  t : ∀{Γ} → {EΓ : tCtx Γ}
    → {T : Type Γ} → {t : Term Γ T}
    → VarCases (term Γ EΓ) (term T t EΓ) (tVar {Γ} EΓ T t)
  -- p : ∀{Γ} → {EΓ : tCtx Γ}
  --   → {T : Type Γ} → {t : Term Γ T}
  --   → VarCases (parametricity Γ EΓ) (parametricity T t EΓ) (tVar {Γ} EΓ T t)
  pt : ∀{Γ} → {EΓ : tCtx Γ}
    → {T : Type Γ} → {t : Term Γ T}
    → VarCases (paraAtType Γ EΓ) (paraAtType T t EΓ) (tVar {Γ} EΓ T t)

{-

There is a problem with this whole concept:
Individual terms can be in any one of the models. But suppose that I want to
write a function which:
-- Inputs a term, does stuff including using parametricity, returns result
Then, it will need to inputs a termPterm and output a term. But there is no
function type which can do that.

-}

data Exp : (Γ : CtxModel) → (T : Model Γ) → Set k where
  EVar : {Γ : CtxModel} → {T : Model Γ} → {Index : Set j}
    → VarCases Γ T Index → Index → Exp Γ T
  App : {Γ₁ Γ₂ Γ₃ : CtxModel} → {A : Model Γ₁} → {B : Model Γ₂} → {C : Model Γ₃}
    → AppCases Γ₁ Γ₂ Γ₃ A B C
    → Exp Γ₁ A → Exp Γ₂ B → Exp Γ₃ C
  parametricity : {Γ : Ctx} → {EΓ : tCtx Γ} → Exp (parametricity Γ EΓ) (parametricity EΓ)
