...
data Exp : Ctx → Type → Set where
  ...
  ⋆ : ∀{Γ} → Exp Γ base
  num : ∀{Γ} → ℕ → Exp Γ Nat
  plus : ∀{Γ} → Exp Γ Nat → Exp Γ Nat → Exp Γ Nat
...
mutual
  data Ne : Ctx → Type → Set where
    ...
    ⋆ : ∀{Γ} → Ne Γ base
    num : ∀{Γ} → ℕ → Ne Γ Nat
    plusLeft : ∀{Γ} → Ne Γ Nat → Nf Γ Nat → Ne Γ Nat
    plusRight : ∀{Γ} → Nf Γ Nat → Ne Γ Nat → Ne Γ Nat

  data Nf : Ctx → Type → Set where
    lambda : ∀{Γ A B} → Nf (Γ , A) B → Nf Γ (A ⇒ B)
    neBase : ∀{Γ} → Ne Γ base → Nf Γ base -- the fact that its only base enforces expanded η form. The above version would also typecheck just fine though.
    neNat : ∀{Γ} → Ne Γ Nat → Nf Γ Nat
...
mutual
  ...
  renNf ren (neBase e) = neBase (renNe ren e)
  renNf ren (neNat e) = neNat (renNe ren e)
...
mutual
  ...
  reify {Γ} {base} g = neBase g
  reify {Γ} {Nat} g = neNat g
