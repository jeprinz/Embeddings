...
Γat : ∀{Γ T} → InCtx Γ T → Ctx
Γat (same {Γ}) = Γ
Γat (next {Γ} {T} x) = Γat x
...
varSub : ∀{Γ A B} → (icx : InCtx Γ A)
  ...
  → (x : InCtx Γ B) → (_≡_ {_} {Σ Type (λ T → InCtx Γ T)} (B , x) (A , icx)) ⊎ (InCtx (subCtx icx) B)
...
mutual
  subv : ∀{Γ T} → ∀{T'} → (icx : InCtx Γ T)
    → (toSub : Nf (Γat icx) T) → Nf Γ T' → Nf (subCtx icx) T'
  subv icx toSub (lambda v) = lambda (subv (next icx) toSub v)
  ...
  subInputs : ∀{Γ T} → ∀{T'} → (icx : InCtx Γ T)
    → (toSub : Nf (Γat icx) T) → (count : ArgCount T')
