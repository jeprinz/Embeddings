...
The idea is to have an augmented shallow embedding, but parametrize Ne and Nf
by Type. The issue is that if Var is parametrized by Type (and also the Context)
has Types in it), then this defeats the point of having the shallow embedding at
all because renamings are no easier than in a normal deep embedding.

So instead, Var and Context will only have shallow types. Instead, the lambda
and Π constructors will have type annotations!

Bizarrely, var will also have to have a type annotation, which seems not right.
...
subType : ∀{n sΓ₁ sΓ₂ T} → {sub : Sᵀ.Sub sΓ₁ sΓ₂} → {Γ₁ : Context sΓ₁} → {Γ₂ : Context sΓ₂}
  → Sub sub Γ₁ Γ₂ → Type {n} Γ₁ T → Type Γ₂ (Sᵀ.subType sub T)

data Ne where
  var : ∀{n} → {SΓ : Sᵀ.Ctx} → {Γ : Context SΓ} → {ST : Sᵀ.Type n SΓ}
    → (T : Type Γ ST)
    → {a : Sᵀ.Term SΓ ST} → (x : Var Γ ST a) → Ne {n} {SΓ} Γ T a
  -- app : ∀{n} → {SΓ : Sᵀ.Ctx} → {Γ : Context SΓ} → {SA : Sᵀ.Type (suc n) SΓ}
  --     → {SB : Sᵀ.Type (suc n) (Sᵀ.cons SΓ SA)}
  --     → {A : Type Γ SA} → {B : Type (Γ , SA) SB}
  --     → ∀{a₁ a₂}
  --     → Ne {suc n} Γ (Π A B) a₁ → (x : Nf {suc n} Γ A a₂)
  --     → Ne {suc n} Γ {λ γ → SB (γ , a₂ γ)} (subType {!   !} B) (Sᵀ.app SA SB a₁ a₂)

data Nf where
  lambda : ∀{n SΓ} → {Γ : Context SΓ} → {SA : Sᵀ.Type (suc n) SΓ}
    → {SB : Sᵀ.Type (suc n) (Sᵀ.cons SΓ SA)} → ∀{a}
    → (A : Type Γ SA) → {B : Type (Γ , SA) SB}
    → Nf (Γ , SA) B a → Nf Γ (Π A B) (Sᵀ.lambda {n} {SΓ} {SA} {SB} a)
  -- TODO: ne should only input on T = fromNe e
  -- ne : ∀{n SΓ Γ ST T t} → Ne {n} {SΓ} Γ {ST} T t → Nf {n} {SΓ} Γ {ST} T t
  ne : ∀{n SΓ Γ ST e t} → Ne {suc n} {SΓ} Γ {ST} (ne e) t → Nf {_} {SΓ} Γ {ST} (ne e) t
...
mutual
  ...
  renType ren U = U
  renType ren (ne e) = ne (renNe ren e)

  renNe : ∀{n sΓ₁ sΓ₂ ST t}
    → {sub : Sᵀ.Sub sΓ₁ sΓ₂} → {Γ₁ : Context sΓ₁} → {Γ₂ : Context sΓ₂}
    → {T : Type Γ₁ ST}
    → (ren : Ren sub Γ₁ Γ₂)
    → Ne {n} Γ₁ T t → Ne Γ₂ (renType ren T) (Sᵀ.subTerm ST sub t)
  renNe ren (var T x) = var (renType ren T) (ren x)

  renNf : ∀{n sΓ₁ sΓ₂ ST t}
    → {sub : Sᵀ.Sub sΓ₁ sΓ₂} → {Γ₁ : Context sΓ₁} → {Γ₂ : Context sΓ₂}
    → {T : Type Γ₁ ST}
    → (ren : Ren sub Γ₁ Γ₂)
    → Nf {n} Γ₁ T t → Nf Γ₂ (renType ren T) (Sᵀ.subTerm ST sub t)
  renNf ren (lambda A e) = lambda (renType ren A) (renNf (liftRen ren) e)
