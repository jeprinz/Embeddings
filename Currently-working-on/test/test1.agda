...
mutual
  ...
  weakenICX (next pre) W (next {_} {_} {_} {_} {T} x)
    = {! InCtx.next {_} {_} {_} {_} { weakenType pre W T } (weakenICX pre W x)  !}

Γat : ∀{n Γ T} → InCtx {n} Γ T → Context
Γat (same {Γ}) = Γ
Γat (next x) = Γat x
...
NOTE: using (Γ' , T) prefix Γ
to represent variables instead of InCtx seems to have some big advantages:
- dont need weakening to define variables
- seems to correspond with debruin LEVELS instead of INDICES
- keeps track of the true context of the variable

-}

mutual
  subCtx : ∀{n Γ Γ'} → {T : Type n Γ'} → Pre (Γ' , T) Γ → Nf Γ' T → Context
  subCtx (same {Γ}) e = Γ
  subCtx (next {Γ₁} {Γ₂} {n} {T} pre) e = (subCtx pre e) , subType pre e T
  subType : ∀{n m Γ Γ'} → {T : Type n Γ'} → (x : Pre (Γ' , T) Γ) → (e : Nf Γ' T)
    → Type m Γ → Type m (subCtx x e)
  subType x e (Π A B) = Π (subType x e A) (subType (next x) e B)
  subType x e (cumu T) = cumu (subType x e T)
  subType x e (type n) = type n
  subType x e (Var x₁) = {!   !}

-- mutual
--   subCtx : ∀{n Γ} → {T : Type n Γ} → (icx : InCtx Γ T) → Nf (Γat icx) T → Context
--   subCtx (same {Γ}) e =  Γ
--   subCtx (next {Γ} {_} {_} {_} {T} icx) e = subCtx icx {! weaken e  !} , {! subType x e T  !}
--
--   subType : ∀{n m Γ} → {T : Type n Γ} → (x : InCtx Γ T) → (e : Nf (Γat x) T)
--     → Type m Γ → Type m (subCtx x e)
