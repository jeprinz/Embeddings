...
Suppose that I had an intrinsic definition of dep thy that works the way
that seems nice.

Then I would have (hereditary) substitution defined mutually with terms.

But, any inductive recursive definition is better thought of as a datatype
with an extra parameter.

So, instead, terms are indexed by a function which describes what happens if
you substitute it?
...
data Type : ℕ → (Γ : Context)
  → (∀{Γ'} → (sub : Sub Γ Γ') → Type Γ')
  → Set -- where
...
-- data Term : (n : ℕ) → (Γ : Context) → (Type n Γ) →
--   → ((sub : Sub Γ Γ') → Term Γ')
--   → Set -- where
