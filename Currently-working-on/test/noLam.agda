...
data Term : {sΓ : S.Ctx} → (Γ : Context sΓ) → (T : S.Type sΓ)
  ...
  app₂ : {sΓ : S.Ctx} → {Γ : Context sΓ} → {A : S.Type sΓ} → {B : S.Type (S.cons sΓ A)} → ∀{s₁ s₂}
      → Term (Γ , A) B s₁ → (x : Term Γ A s₂)
      → Term Γ (λ γ → B (γ , s₂ γ )) (λ γ → s₁ (γ , s₂ γ)) -- (S.app s₁ s₂)
  -- applying to multiple arguments needs to be handled?

-- (λ f . f U) (λ x . x)
e1 : Term ∅ (λ _ → Set₁) S.U₀
e1 = app (lambda (app (var same) U₀)) (lambda (var same))

-- (0 [0 ↦ 0]) [0 ↦ 0]
-- (f [x ↦ U]) [f ↦ x]
-- e2 : Term ∅ (λ _ → Set₁) _
-- e2 = app₂ (app₂ (var same) U₀) {! var same  !}
e2 : Term ∅ (λ _ → Set₁) S.U₀
e2 = app₂ (app (var same) U₀) {! var same  !}

-- NOTE: if I can know Γ, why can't I know the spines of the Π types? same thing.
