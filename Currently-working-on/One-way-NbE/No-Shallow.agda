...
The idea of this file is to be ok with turning off any termination and
positivity checks, and just see if I can get it to typecheck with only
Exp and Sem.

The Context will be a list of SemT, and the type parameter on Exp will be
a SemT.

As a guiding principle, should untype to standard untyped NbE.
...
data VarT : (Γ : Context) → SemT Γ → Set₁ where
...
RenT : Context → Context → Set₁
RenT Γ₁ Γ₂ = ∀{T} → VarT Γ₁ T → VarT Γ₂ {! T  !}
...
-- ERen Γ₁ Γ₂
  -- = ∀{T t} → Var Γ₁ T t → Var Γ₂ {! T  !} {!   !}
--                                  ↑ need a self type here
