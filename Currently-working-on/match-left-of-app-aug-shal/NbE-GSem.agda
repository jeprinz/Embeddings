...
module Esucn (n : ℕ)
  ...
  mutual
    ...
    GESemTsucn : {SΓ : Ctx} → (Γ : Context SΓ) → (T : Type (suc n) SΓ) → Set₁
    GESemTsucn {SΓ} Γ T = ∀{SΓ'} → {Γ' : Context SΓ'}
      → {ren : Sub SΓ SΓ'} → ERen ren Γ Γ' → ESemTsucn Γ' (subType ren T)
    ...
    GSemsucn : ∀{SΓ} → (Γ : Context SΓ) → (T : Type (suc n) SΓ)
      → Term SΓ T → ESemTsucn Γ T → Set₁
    GSemsucn {SΓ} Γ T t T
...
mutual
  ...
  reifyT {suc n} (Ecumu T) = Ecumu (reifyT T)
  reifyT {suc n} (EΠ A B) = EΠ (reifyT (A EidRen))
    (reifyT (B Eweaken1Ren (nApp (A Eweaken1Ren) (Evar same))))
